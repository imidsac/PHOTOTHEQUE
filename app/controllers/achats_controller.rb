class AchatsController < ApplicationController
  ...
  def show
    ...
    if @achat.type_ac == 'C'
    @alignes = @achat.alignes.select("cadre_id,numerobaguete,qte,qtelivre,prix_u,montant, alignes.id, alignes.etat").joins(:cadre).where("cadre_id != -1")
    else
    @alignes = @achat.alignes.select("article_id,name,reference,qte,qtelivre,prix_u,montant, alignes.id, alignes.etat, alignes.achat_id").joins(:article).where("article_id != -1")
    end
